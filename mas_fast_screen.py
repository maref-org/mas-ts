            ...
            try:
                ...
                if isinstance(parsed, dict) and "overall_passed" in parsed:
                    if not parsed["overall_passed"]:
                        result["status"] = "FAIL"
                        result["error"] = f"overall_passed=false in output"
                    else:
                        result["status"] = "PASS"
...
def run_mock_calibration(golden_dir, mock_dir, thresholds=None):
    cmd = [
        sys.executable, "mock_calibrate.py",
...
def print_summary(report):
    status_color = {"PASS": "green", "FAIL": "red", "TIMEOUT": "yellow", "ERROR": "red", "WARNING": "yellow"}
    ...
    overall_icon = {"PASS": "✅", "FAIL": "❌", "WARNING": "⚠️"}.get(report["overall_status"], "❓")
...
def main():
    ...
    parser.add_argument("--golden-dir", help="Golden Trajectories directory for drift calibration")
    parser.add_argument("--mock-dir", help="Mock output directory for drift calibration")
    parser.add_argument("--block", action="store_true", help="Exit with error code on failure")
    ...
    with Progress(
        ...
        if args.task_file and Path(args.task_file).exists():
            task2 = progress.add_task("[cyan]Layer 3: Mock LLM Action Test...", total=None)
            stage2 = run_mock_llm_test(args.task_file, args.policy)
            stages.append(stage2)
            progress.update(task2, description=f"[{'green' if stage2['status']=='PASS' else 'red'}]{stage2['status']} ({stage2['duration_ms']}ms)")
        else:
            progress.add_task("[yellow]Skipped: Mock LLM test (no --task-file)", total=None)
        ...
        if args.golden_dir and args.mock_dir:
            ...
            progress.update(task3, description=f"[{'green' if stage3['status']=='PASS' else 'red'}]{stage3['status']} ({stage3['duration_ms']}ms)")
        else:
