...
def run_layer1_static_audit(card, schema_path=None):
    ...
    if not backend_loc:
        findings.append(
            {
                "severity": "CRITICAL",
                "category": "model_backend_location",
                "detail": "Missing model_backend_location",
            }
        )
        score -= 15
        ...
        findings.append(
            {
                ...
                "detail": "No capabilities declared",
            }
        )
        score -= 20
...
def run_layer2_inference_metrics(card):
    ...
    inference_score = (
        reasoning_score * 0.4 + coding_score * 0.35 + multilingual_score * 0.25
    )
...
def run_layer3_action_metrics(card, tasks=None):
            ...
            for task in task_list:
                ...
                if all(t in declared_tools for t in expected):
                    ...
                    uncovered_tasks.append(
                        {"task_id": task.get("id", "?"), "missing_tools": missing}
                    )
    ...
    return {
        ...
        "summary": {
            ...
            "schema_completeness": f"{(1 - schema_issues / (len(capabilities) * 2)) * 100:.0f}%"
            if capabilities
            else "0%",
...
def run_layer5_mas_dimension(card, tasks=None):
    ...
    weighted_score = sum(
        dimension_scores[dim] * MAS_DIMENSIONS[dim]["weight"] for dim in MAS_DIMENSIONS
    )
...
def print_report(report):
    ...
    for layer in report["layers"]:
        ...
        print(
            f"  {emoji} Layer {layer['layer']}: {layer['name']:<25} {layer['score']:>5.1f}/100  Grade {layer['grade']}"
        )
    ...
    if report["recommendations"]:
        ...
        for rec in report["recommendations"][:8]:
            print(
                f"  [{rec['priority']}] L{rec['layer']} {rec['category']}: {rec['recommendation'][:80]}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="MAS-TS-001 Full-Run Evaluation Pipeline (Layer 1-5)"
    )
    parser.add_argument(
    ...
    schema_path = (
        args.schema
        if args.schema != str(DEFAULT_SCHEMA)
        else select_schema(card, args.schema)
    )
