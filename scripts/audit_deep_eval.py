...
def evaluate_card(card_path: Path) -> dict:
    ...
    # 打印关键发现
    critical_high = [f for f in d1.get('findings', []) if f.get('severity') in ('CRITICAL', 'HIGH')]
    if critical_high:
    ...
    critical_high = [f for f in d2.get('findings', []) if f.get('severity') in ('CRITICAL', 'HIGH')]
    if critical_high:
        print(f"  Issues: {len(critical_high)}")
        for f in critical_high[:3]:
            print(f"    [{f['severity']}] {f['detail']}")
    ...
    critical_high = [f for f in d3.get('findings', []) if f.get('severity') in ('CRITICAL', 'HIGH')]
    if critical_high:
        print(f"  Issues: {len(critical_high)}")
        for f in critical_high[:3]:
            print(f"    [{f['severity']}] {f['detail']}")
    ...
    critical_high = [f for f in d4.get('findings', []) if f.get('severity') in ('CRITICAL', 'HIGH')]
    if critical_high:
        print(f"  Issues: {len(critical_high)}")
        for f in critical_high[:3]:
            print(f"    [{f['severity']}] {f['detail']}")
    ...
    critical_high = [f for f in d5.get('findings', []) if f.get('severity') in ('CRITICAL', 'HIGH')]
    if critical_high:
        print(f"  Issues: {len(critical_high)}")
        for f in critical_high[:3]:
            print(f"    [{f['severity']}] {f['detail']}")
    ...
    # 计算加权总分
    total_score = sum(
        results[domain]["score"] * DOMAIN_WEIGHTS[domain]
