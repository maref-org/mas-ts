...
MODEL_QUALITY_DB = {
    "claude-opus-4":     {"reasoning": 95, "coding": 95, "multilingual": 90, "instruction": 92, "tier": "premium"},
    "claude-sonnet-4":   {"reasoning": 88, "coding": 90, "multilingual": 85, "instruction": 88, "tier": "premium"},
    "claude-haiku-3.5":  {"reasoning": 78, "coding": 80, "multilingual": 80, "instruction": 82, "tier": "fast"},
    "gpt-4o":            {"reasoning": 85, "coding": 88, "multilingual": 90, "instruction": 85, "tier": "premium"},
    "gpt-4-turbo":       {"reasoning": 80, "coding": 82, "multilingual": 85, "instruction": 80, "tier": "premium"},
    "deepseek-chat-v3":  {"reasoning": 85, "coding": 85, "multilingual": 78, "instruction": 80, "tier": "value"},
    "qwen-max":          {"reasoning": 82, "coding": 78, "multilingual": 88, "instruction": 82, "tier": "value"},
}

E2E_SCENARIOS = [
    {"name": "Code Search & Fix",            "required_tools": ["grep", "file_read", "file_edit"],       "expected_agents": 1,  "expected_steps": "3-5"},
    {"name": "New Feature Implementation",   "required_tools": ["file_read", "file_write", "glob", "grep"], "expected_agents": "1-2", "expected_steps": "8-15"},
    {"name": "Bug Investigation",            "required_tools": ["grep", "file_read", "bash", "web_search"], "expected_agents": "1-2", "expected_steps": "5-10"},
    {"name": "Code Review & Refactor",       "required_tools": ["file_read", "file_edit", "glob"],          "expected_agents": 1,  "expected_steps": "6-12"},
    {"name": "Documentation Generation",     "required_tools": ["file_read", "file_write", "web_search"],   "expected_agents": 1,  "expected_steps": "4-8"},
    {"name": "Web Research & Integration",   "required_tools": ["web_search", "web_fetch", "file_write"],   "expected_agents": "1-2", "expected_steps": "6-12"},
    {"name": "Multi-file Refactoring",       "required_tools": ["file_read", "file_edit", "glob", "grep"],  "expected_agents": "1-2", "expected_steps": "8-20"},
    {"name": "Test-Driven Development",      "required_tools": ["file_write", "file_read", "bash"],         "expected_agents": 1,  "expected_steps": "8-15"},
]
...
def run_model_quality(card):
    ...
    if model_key:
        ...
        composite = mq["reasoning"] * 0.35 + mq["coding"] * 0.30 + mq["multilingual"] * 0.20 + mq["instruction"] * 0.15
        findings.append({
            "severity": "INFO", "category": "model_quality",
        ...
        findings.append({
            "severity": "WARNING", "category": "model_unknown",
...
def run_tool_coverage(card):
    ...
    if missing_core:
        findings.append({
            "severity": "WARNING", "category": "missing_core_tools",
    ...
    findings.append({
        "severity": "INFO", "category": "tool_coverage",
    ...
    if schema_issues:
        findings.append({
            "severity": "WARNING", "category": "schema_incomplete",
...
def run_task_completion(card, golden_trajectory=None, mock_trajectory=None):
    ...
    if not golden_trajectory or not mock_trajectory:
        findings.append({
            "severity": "WARNING", "category": "task_completion_skipped",
    ...
    golden_events = golden_trajectory if isinstance(golden_trajectory, list) else golden_trajectory.get("events", [])
    mock_events = mock_trajectory if isinstance(mock_trajectory, list) else mock_trajectory.get("events", [])

    golden_sigs = [extract_tool_signature(e) for e in golden_events if extract_tool_signature(e)]
    mock_sigs = [extract_tool_signature(e) for e in mock_events if extract_tool_signature(e)]

    if not golden_sigs:
        findings.append({"severity": "WARNING", "category": "task_completion", "detail": "No tool calls in golden trajectory"})
        return 0.0, findings
    ...
    golden_routes = [extract_routing_decision(e) for e in golden_events if extract_routing_decision(e)]
    mock_routes = [extract_routing_decision(e) for e in mock_events if extract_routing_decision(e)]
    route_match = 0.0
    ...
    score = seq_sim * 25 + set_sim * 25 + param_match * 20 + route_match * 15 + output_correctness * 15
    score = max(0, min(100, score))

    findings.append({
        "severity": "INFO", "category": "task_completion",
...
def run_e2e_scenarios(card):
    ...
    for scenario in E2E_SCENARIOS:
        ...
        if not missing:
            completed += 1
            scenario_score = 100
            findings.append({"severity": "INFO", "category": "e2e_scenario", "detail": f"[PASS] {scenario['name']}"})
        elif len(available) >= len(required) * 0.5:
            partial += 1
            scenario_score = 50
            findings.append({"severity": "WARNING", "category": "e2e_scenario", "detail": f"[PARTIAL] {scenario['name']} — missing {', '.join(sorted(missing))}"})
        else:
            failed += 1
            scenario_score = 0
            findings.append({"severity": "HIGH", "category": "e2e_scenario", "detail": f"[FAIL] {scenario['name']} — missing critical {', '.join(sorted(missing))}"})
        scenario_scores.append(scenario_score)

    total = len(E2E_SCENARIOS)
    e2e_score = sum(scenario_scores) / total if total else 0
    findings.append({
        "severity": "INFO", "category": "e2e_summary",
...
def run_d2(card, golden_trajectory=None, mock_trajectory=None):
    ...
    task_score, task_findings = run_task_completion(card, golden_trajectory, mock_trajectory)
    e2e_score, e2e_findings = run_e2e_scenarios(card)

    all_findings = model_findings + tool_findings + task_findings + e2e_findings

    d2_score = model_score * 0.25 + tool_score * 0.20 + task_score * 0.30 + e2e_score * 0.25
    ...
    return {
        ...
        "summary": {
            ...
            "declared_tools_count": len({cap["skill_id"] for cap in card.get("capabilities", [])}),
        },
    }
