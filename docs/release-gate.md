...
**版本**: v0.2 | 可执行 10 项发布门禁 (Phase 7.1)

> 对应 `产品级发布全量验收标准与评审流程手册.md` 的五 Gate 模型，精炼为 MAS-TS CLI 项目可执行的 10 项检查。每项检查映射到具体命令或文档化的人工审批。

---

## 可执行清单

运行 `python3 scripts/release_gate_check.py` 自动检查 G1-G3 自动项；G0 项需人工审批。
...
# 检查 10 项中已勾选的数量 (legacy checkbox mode)
